...
# ---------------------------------------------------------------------------
# Session transport config (client.py)
# ---------------------------------------------------------------------------

class TestClientTransportConfig:
    """Tests for curl_cffi session options applied by Client."""

    def test_forces_ipv4_when_configured(self):
        """Windows default should be able to pin curl_cffi to IPv4."""
        with (
            patch("perplexity.client.PPLX_IP_RESOLVE", "ipv4"),
            patch("perplexity.client.requests.Session") as mock_session_cls,
        ):
            ...
            Client({"csrf": "test", "session": "test"})

            session_kwargs = mock_session_cls.call_args.kwargs
            assert session_kwargs["curl_options"][CurlOpt.IPRESOLVE] == CurlIpResolve.V4

    def test_auto_mode_leaves_ip_resolution_unset(self):
        """Auto mode should not inject an IP family preference."""
        with (
            patch("perplexity.client.PPLX_IP_RESOLVE", "auto"),
            patch("perplexity.client.requests.Session") as mock_session_cls,
        ):
            mock_session = MagicMock()
            mock_session_cls.return_value = mock_session
            ...
            session_kwargs = mock_session_cls.call_args.kwargs
            assert "curl_options" not in session_kwargs
