...
export function TokenTable({
    ...
    list.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'state':
          ...
          break
        case 'quota':
          cmp = getQuotaSortValue(a) - getQuotaSortValue(b)
          break
        case 'requests':
          cmp = (a.request_count || 0) - (b.request_count || 0)
          break
        case 'last_check':
          cmp = (a.last_check_at || '').localeCompare(b.last_check_at || '')
          break
      }
  ...
  const handleClientAction = async (action: string, id: string) => {
    ...
    if (resp.status === 'ok') {
      onToast(`Client ${id} ${action}d`, 'success')
      onRefresh()
    } else {
  ...
  const handleTestClient = async (id: string) => {
    ...
    try {
      ...
      if (resp.status === 'ok') {
        onToast(`Test ${id} passed`, 'success')
        onRefresh()
      } else {
        onToast(resp.error || resp.message || 'Test failed', 'error')
      }
    } finally {
      setTestingIds((prev) => {
  ...
  return (
    <div className="rounded-xl border border-border-subtle bg-surface">
      <div className="p-4 md:p-6">
        <div className="mb-4 flex flex-col items-start justify-between gap-4 border-b border-border-subtle pb-4 md:flex-row md:items-center">
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-border-subtle">
                  <th
                    className="cursor-pointer select-none p-3 text-xs font-medium text-text-muted"
                  ...
                  <th
                    className="cursor-pointer select-none p-3 text-xs font-medium text-text-muted"
                    onClick={() => toggleSort('last_check')}
                  >
                    Last Check
                    <SortIcon field="last_check" />
                  </th>
                  <th className="p-3 text-right text-xs font-medium text-text-muted">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="text-sm">
                {filteredAndSorted.map((c) => {
                  const isDisabledInProOnlyMode =
                    !fallbackToAuto && c.state === 'exhausted'
                  const errors = getErrorCount(c)
                  return (
                    <tr
                      key={c.id}
                      className={`border-border-subtle/50 border-b transition-colors ${
                    ...
                    >
                      ...
                      <td className="p-3">
                        {!c.enabled ? (
                          <span className="inline-flex items-center gap-1.5 rounded-full bg-elevated px-2 py-0.5 text-xs text-text-muted">
                          ...
                          <span className="inline-flex items-center gap-1.5 rounded-full bg-error/10 px-2 py-0.5 text-xs text-error">
                          ...
                          <span className="inline-flex items-center gap-1.5 rounded-full bg-info/10 px-2 py-0.5 text-xs text-info">
                      ...
                      <td className="p-3 font-mono text-xs text-text-muted">
                        {c.last_check_at
                          ? new Date(c.last_check_at).toLocaleTimeString()
                          : '-'}
                      </td>
                        ...
                        <div className="flex justify-end gap-0.5">
                          ...
                          <button
                            className={`rounded-md p-1.5 transition-colors ${
                              !downloadingIds.has(c.id)
                                ? 'text-text-muted hover:bg-elevated hover:text-accent'
                          ...
                          {c.enabled ? (
                            <button
                              className="rounded-md p-1.5 text-text-muted transition-colors hover:bg-elevated hover:text-warning"
                              onClick={() =>
                                handleClientAction('disable', c.id)
                              }
                              title="Disable"
                            >
                              <svg
                            ...
                            <button
                              className="rounded-md p-1.5 text-text-muted transition-colors hover:bg-elevated hover:text-success"
                          ...
                          {/* Delete */}
                          <button
                            className="rounded-md p-1.5 text-text-muted transition-colors hover:bg-elevated hover:text-error"
    ...
    <div className="flex flex-col gap-0.5">
      {items.map((item) => (
        <div key={item.label} className="flex items-center gap-1.5 text-xs">
          <span className="w-14 text-text-muted">{item.label}</span>
