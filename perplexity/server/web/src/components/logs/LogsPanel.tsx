...
function highlightMatch(line: string, query: string): JSX.Element {
  ...
  return (
    <>
      {line.slice(0, index)}
      <span className="rounded bg-warning/30 px-0.5">
      ...
      <div className="flex flex-col gap-4 rounded-xl border border-border-subtle bg-surface p-4 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <select
            ...
            className={`rounded-lg border border-border bg-elevated px-2.5 py-1 text-xs ${
              isAutoRefresh
                ? 'text-text-secondary'
          ...
          <select
            ...
            className="rounded-lg border border-border bg-elevated px-2.5 py-1 text-xs text-text-secondary focus:outline-none focus:ring-1 focus:ring-accent"
          >
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <input
              ...
              className="w-48 rounded-lg border border-border bg-elevated px-3 py-1.5 text-sm text-text-secondary placeholder:text-text-muted focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent"
            />
            {searchQuery && (
              <button
            ...
            <tbody>
              {displayLines.map((line, i) => (
                <tr key={i} className="hover:bg-elevated/50">
                  <td className="w-12 select-none border-r border-border-subtle px-3 py-0.5 text-right align-top text-text-muted">
                    {i + 1}
                  </td>
                  <td
                    className={`whitespace-pre-wrap px-3 py-0.5 ${getLineClass(
                      line
                    )}`}
                  >
