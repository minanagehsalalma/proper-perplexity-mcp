...
_ADMIN_WEB_DIR = pathlib.Path(__file__).parent / "web"
_ADMIN_DIST_DIR = _ADMIN_WEB_DIR / "dist"
_ADMIN_INDEX_PATH = _ADMIN_DIST_DIR / "index.html"


def _admin_build_missing_response() -> HTMLResponse:
    """Return a helpful response when the admin frontend has not been built yet."""
    return HTMLResponse(
        """
        <!doctype html>
        <html lang="en">
          <head>
            <meta charset="utf-8">
            <title>Admin UI Build Missing</title>
          </head>
          <body>
            <h1>Admin UI build missing</h1>
            <p>Build the frontend before opening <code>/admin/</code>.</p>
            <pre>cd perplexity/server/web
npm ci
npm run build</pre>
          </body>
        </html>
...
async def admin_page_index(request: Request):
    ...
    if not _ADMIN_INDEX_PATH.is_file():
        return _admin_build_missing_response()
...
async def admin_static(request: Request):
    ...
    if not _ADMIN_DIST_DIR.is_dir():
        return _admin_build_missing_response()

    file_path = _ADMIN_DIST_DIR / path

    # Security check: ensure path is within dist directory
    try:
        file_path = file_path.resolve()
        dist_dir = _ADMIN_DIST_DIR.resolve()
